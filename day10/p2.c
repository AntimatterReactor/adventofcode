/*
    This program acts as a solution to the day 10 part 2 of AoC 2023
...
{
    resultgrid[cpos.y - 1][cpos.x - 1] = 'S';
    pos_t rpos = {0, 0};
...
int main(void)
{
    ...
    fclose(fp);
    printf("%lu; max: %u, %u; min: %u, %u\n", process_grid(spos), max_pos.x, max_pos.y, min_pos.x, min_pos.y);
    // for (size_t i = 0; i < GRID_SIZE; ++i)
    // {
